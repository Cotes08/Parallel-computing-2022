...
void ImprimirResultadosFichero(char *nombreC, char *fich, int filas, int columnas, double segundos, double timeLeer);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    matrizResult = ReservaMatriz(filas, columnas);
    ...
    */
    ...
    for (i = 1; i <= filas; i++)
    {
        ...
        {
            matrizResult[i - 1][j - 1] = CalcularMedia(matriz, i, j);
        }
    }
    // Escribir media
    Escribir(matrizResult, filas, columnas, 1);
    ...
    for (i = 1; i <= filas; i++)
    {
        for (j = 1; j <= columnas; j++)
        {
            matrizResult[i - 1][j - 1] = CalcularMediana(matriz, i, j);
        }
    }
    // Escribir mediana
    Escribir(matrizResult, filas, columnas, 2);
    ...
    for (i = 1; i <= filas; i++)
    {
        for (j = 1; j <= columnas; j++)
        {
            matrizResult[i - 1][j - 1] = CalcularSobel(matriz, i, j);
        }
    }
    // Escribir SOBEL
    Escribir(matrizResult, filas, columnas, 3);

    segundos = (double)(clock() - cl) * 1000 / CLOCKS_PER_SEC;
    timeLeer = (double)(endLeer - beginLeer) * 1000 / CLOCKS_PER_SEC;
    ImprimirResultadosFichero(nombreC, fich, filas, columnas, segundos, timeLeer);
    return 0;
}
...
void ImprimirResultadosFichero(char *nombreC, char *fich, int filas, int columnas, double segundos, double timeLeer)
{
    ...
    fprintf(fr, "Sentencia de ejecucion: '%s' '%s' '%i' '%i' \n", nombreC, fich, filas, columnas);
